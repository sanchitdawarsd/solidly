...
async function main(network) {
  ...
  await mim.mint(deployerAddress, ethers.BigNumber.from("1000000000000000000000000000000"));
  const dai = await Token.deploy('DAI', 'DAI', 18, deployerAddress);
  await dai.mint(deployerAddress, ethers.BigNumber.from("1000000000000000000000000000000"));

  await ust.deployed();
  await mim.deployed();
  await dai.deployed();

  console.log("TOKENS ADDRESSES: UST,MIM,DAI respectivly:", ust.address,mim.address,dai.address)
  ...
  //DEPLOYING PAIRS
    ...
    await baseV1Router.addLiquidity(mim.address, ust.address, true, mim_1, ust_1, 0, 0, deployerAddress, Date.now());
    await dai.approve(baseV1Router.address, dai_1);
    await ust.approve(baseV1Router.address, ust_1);
    await baseV1Router.addLiquidity(dai.address, ust.address, false, dai_1, ust_1, 0, 0, deployerAddress, Date.now());
    await mim.approve(baseV1Router.address, mim_1);
    await dai.approve(baseV1Router.address, dai_1);
    await baseV1Router.addLiquidity(mim.address, dai.address, true, mim_1, dai_1, 0, 0, deployerAddress, Date.now());
    ...
    console.log("PAIRS addresses MIM,UST & DAI,UST & MIM,DAI",address,address2,address3)
